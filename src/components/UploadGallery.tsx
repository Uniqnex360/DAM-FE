...
export function UploadGallery({ userId, allUsers }: UploadGalleryProps) {
  ...
  const handleDeleteSession = async (sessionId: string) => {
    try {
      await supabase.from("images").delete().eq("upload_id", sessionId);
      await supabase.from("uploads").delete().eq("id", sessionId);
 ...
 const handleImageClick = (image: any, session: any) => {
  ...
  return (
    ...
    <ImageGrid
      ...
      headerDescription="View your uploaded images and their processing status"
      headerIcon={Package}
