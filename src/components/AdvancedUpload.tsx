  ...
  export function AdvancedUpload() {
    ...
    const handleUpload = async () => {
      ...
      case 'original':
        // User selected "Resize" checkbox but chose "Original" mode.
        // We remove the resize operation so backend keeps original size.
        console.log("Resize mode: Original (Skipping resize)");
        operationsToSend = operationsToSend.filter(op => op !== 'resize');
        break;

      case 'preset':
      case 'custom':
        // Both modes utilize the resizeDims state directly
        console.log(`Resize mode: ${activeResizeMode} (${resizeDims.width}x${resizeDims.height})`);
      ...
      case 'percentage':
        console.log(`Resize mode: Percentage (${resizePercentage}%)`);
        
        // Calculate dimensions for the specific image being processed
        ...
        if (originalImage?.originalWidth && originalImage?.originalHeight) {
          ...
          // Fallback for when client-side dims aren't available
    ...
    <div className="mt-2 ml-8 p-4 bg-white border border-slate-200 rounded-lg shadow-sm space-y-4">
      ...
      {/* 1. SELECTION BUTTONS - MUTUALLY EXCLUSIVE */}
