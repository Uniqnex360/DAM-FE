...
export function AdvancedUpload() {
  ...
  const handleProcessBatch = async () => {
    ...
    if (selectedProcessing.includes("smart-frame") || images.some(img => 
    (img.selectedOps?.length ? img.selectedOps : selectedProcessing).includes("smart-frame")
  )) {
    if (!smartFrameOptions.width || !smartFrameOptions.height) {
      toast.error("Please enter valid width and height for smart frame");
      return;
    }
  }
    ...
    if (anyImageNeedsLineDiagram) {
      const imagesWithoutMeasurements = images.filter((img) => {
        const ops = img.selectedOps?.length
          ...
          : selectedProcessing;
        return (
          ops.includes("line-diagram") &&
          !lineDiagramResults.some(
            (r) => r.imageId === img.id && r.measurements.length > 0,
          )
        );
      });
            ...
            {images.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-96 text-center">
                ...
                <p className="text-sm text-slate-400 max-w-sm">
                  Drag and drop files or use the import options on the left to
                  add images
                </p>
              </div>
            ) : (
              ...
              <div className="flex items-center space-x-6">
                ...
                {!autoDetect && (
                  <>
                    <button
                      onClick={() => {
                        const allSelected = PROCESSING_OPTIONS.every((o) =>
                          selectedProcessing.includes(o.id),
                        );
                        if (allSelected) {
                                ...
                                {PROCESSING_OPTIONS.filter((op) =>
                                  ...
                                  return (
                                    <button
                                      ...
                                      type="button"
                                      onClick={() =>
                                        toggleImageOperation(img.id, op.id)
                                      }
                                      className={cn(
                            ...
                            backgroundColor !== "transparent" ? (
                              ...
                              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 mb-3">
                                <p className="text-xs text-slate-500">
                                  Background will be removed and kept
                                  transparent. Toggle "Change Color" to fill
                                  with a solid color.
                                </p>
                              </div>
                            )}
                                ...
                                <div className="flex flex-wrap gap-2 mb-3">
                                  {[
                                    ...
                                    <button
                                      key={ratio}
                                      onClick={() => {
                                        const uncropped = images.find((img) => {
                                          const imgOps = img.selectedOps?.length
                                            ? img.selectedOps
                                            : selectedProcessing;
                                          return (
                                            imgOps.includes("crop") &&
                                            !img.isCropped
                                          );
                                        });
                                        if (uncropped) {
                              ...
                              {images.filter((img) => {
                                ...
                                <div className="text-center py-4">
                                  ...
                                  <p className="text-[10px] text-slate-400 mb-3">
                                    Select images to crop from per-image
                                    overrides or add crop globally
                                  </p>
                                  {images.length > 1 &&
                                  showPerImageOverrides ? (
                                    <button
                                      onClick={() =>
                                        setShowPerImageOverrides(true)
                                      }
                              ...
                              <div className="text-center py-4">
                                ...
                                {images.length > 1 && showPerImageOverrides ? (
                                  ...
                                  <button
                                    onClick={() => {
                                      setSelectedProcessing((prev) =>
                                        prev.includes("line-diagram")
                                          ? prev
                                          : [...prev, "line-diagram"],
                                      );
                                      toast.success(
                                        "Line diagram added globally",
                                      );
                                    }}
                                    className="px-3 py-1.5 text-xs bg-purple-50 text-purple-600 border border-purple-200 rounded-md hover:bg-purple-100"
                                  >
                            ...
                            >
                              <div className="w-16 h-16 rounded-xl overflow-hidden bg-white border border-slate-200 flex-shrink-0 shadow-sm">
                                <img
                                  ...
                                  onError={(e) => {
                                    e.currentTarget.src =
                                      'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100"><rect width="100" height="100" fill="%23f0f0f0"/><text x="50" y="55" text-anchor="middle" fill="%23999" font-size="12">No preview</text></svg>';
                                  }}
                                />
                              </div>
                              <div className="flex-1 min-w-0">
                                    ...
                                    {hasCustomOps ? (
                                      <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full flex items-center gap-1">
                                          ...
                                          Custom ({activeOpsCount} of{" "}
                                          {selectedProcessing.length} ops)
                                        </span>
                                        <button
                                          type="button"
                                          onClick={() => {
                ...
                <>
                  ...
                  <button
                    onClick={async () => {
                      try {
                        const uploadId =
                          processedResults[0]?.metadata?.upload_id;
