...
import { UserPlus, Edit, Trash2, Shield, User as UserIcon, X, Check } from "lucide-react";
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-md mb-8 border border-gray-200">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <input type="text" placeholder="Full Name" value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} className="border p-2 rounded" />
            <input type="password" placeholder={editingId ? "Leave blank to keep same" : "Password"} onChange={e => setFormData({...formData, password: e.target.value})} required={!editingId} className="border p-2 rounded" />
          ...
          <div className="mt-4 flex justify-end gap-2">
            ...
            <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded">Save User</button>
          </div>
        </form>
      )}
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-4">
                   {u.is_active ? <Check className="text-green-500" /> : <X className="text-red-500" />}
                </td>
                <td className="p-4 text-right flex justify-end gap-2">
                   <button onClick={() => handleEdit(u)} className="p-2 hover:bg-blue-50 text-blue-600 rounded"><Edit size={16} /></button>
                   <button onClick={() => handleDelete(u.id)} className="p-2 hover:bg-red-50 text-red-600 rounded"><Trash2 size={16} /></button>
                </td>
              </tr>
            ))}
          </tbody>
