...
const IMAGE_FUNCTIONS = [
  {
    ...
    description: "Automatically resize images to exact pixel dimensions required by the platform",
    icon: Maximize2,
    color: "sky",
  },
  {
    ...
    description: "Detect and correct background colors to meet marketplace requirements",
    icon: Palette,
    color: "emerald",
  },
  {
    ...
    description: "Crop or pad images to the exact aspect ratio required per platform and image slot",
    icon: Crop,
    color: "amber",
  },
  {
    ...
    description: "AI-powered check and auto-correction to ensure a pure white (#FFFFFF) background",
    icon: CheckCircle,
    color: "teal",
  },
  {
    ...
    description: "Auto-generate branded infographic images with product features and spec callouts",
    icon: ImageIcon,
    color: "orange",
  },
  {
    ...
    description: "Detect and cleanly remove supplier watermarks, logos, or unwanted overlays using AI inpainting",
    icon: Sparkles,
    color: "rose",
  },
  {
    ...
    description: "Detect forbidden text, pricing, or promotional text overlaid on images and flag or remove them",
    icon: Type,
    color: "violet",
  },
  {
    ...
    description: "Automatically generate color, size, and style variant images from a single product shot",
    icon: Layers,
    color: "cyan",
  },
];
...
export function MarketplaceSyndication() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
       <div className="bg-white rounded-2xl border border-slate-200 p-2 flex items-center justify-between shadow-sm">
        <div className="flex items-center">
          {/* Active Tab: Browse Marketplaces */}
          <button className="flex items-center space-x-2 px-6 py-2.5 border border-blue-200 bg-blue-50 rounded-xl transition-all">
            ...
            <span className="font-bold text-blue-700 text-sm">Browse Marketplaces</span>
          </button>

          {/* Inactive Tab: Import for Marketplace */}
          <button 
            // You can add navigation logic here to go to the AdvancedUpload page
            className="flex items-center space-x-2 px-6 py-2.5 text-slate-600 hover:text-slate-700 transition-colors rounded-xl hover:bg-slate-50"
          >
      ...
      <nav className="flex items-center space-x-2 text-sm font-medium mb-6">
        ...
        {/* Step 2: Visible if a country is selected */}
        {selectedCountry && (
          <>
        ...
        {/* Step 3: Visible if a marketplace is selected */}
        {selectedMarketplace && (
          <>
            <ChevronRight className="w-4 h-4 text-slate-300" />
            <button
  ...
  className={`p-2.5 rounded-xl border transition-all ${
    isSelected
      ? "bg-sky-500 border-sky-400 text-white" // Selected state (Blue)
      : `bg-white border-slate-100 ${getColorClasses(func.color)} shadow-sm` // Dynamic color state
  }`}
>
